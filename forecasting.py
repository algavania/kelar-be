...
with open('sarima_models.pkl', 'rb') as file:
    models = joblib.load(file)

# Define the URL to your Google Sheets CSV export
google_sheet_url = 'https://docs.google.com/spreadsheets/d/11Tp3mY7JkAnLTMw4JIXu5cmaMpIOx7xcjgHwt6xPT80/export?format=csv&gid=0'
...
expected_columns = ['Date and Time', 'Humidity (%)', 'Temperature (°C)', 'CO2 (ppm)', 'CO (ppm)', 'PM2.5 Density (ug/m3)']
if not all(col in df.columns for col in expected_columns):
    raise KeyError("Some expected columns are missing from the DataFrame.")

# Convert 'Date and Time' column to datetime
df['Date and Time'] = pd.to_datetime(df['Date and Time'])

# Convert numeric columns to proper numeric types
numeric_cols = ['Humidity (%)', 'Temperature (°C)', 'CO2 (ppm)', 'CO (ppm)', 'PM2.5 Density (ug/m3)']
for col in numeric_cols:
    df[col] = pd.to_numeric(df[col].str.replace(',', '.'), errors='coerce')  # Handle any commas and convert to float

# Define the categorize_time_of_day function
...
# Apply the categorize_time_of_day function to create 'Time of Day' column
...
non_numeric_cols = ['Date', 'Time of Day']

# Perform aggregation on numeric columns
grouped = df.groupby(non_numeric_cols)[numeric_cols].agg('mean').reset_index()

# Check the columns in the grouped DataFrame
print("Columns in grouped DataFrame:", grouped.columns)
print(grouped)
...
# Define the sarima_forecast function
...
# Initialize models dictionary and predictions DataFrame
models = {}
predictions = pd.DataFrame()

# Train SARIMA models for each feature and make predictions
for feature in features:
    feature_data = grouped.set_index('Date')[feature]
    min_value = min_values[feature]
    model_fit, forecast_mean = sarima_forecast(feature_data, order, seasonal_order, steps=4, min_value=min_value)
    predictions[feature] = forecast_mean
    models[feature] = model_fit

# Generate future dates and times of day
...
# Define the calculate_aqi function
...
# Define the categorize_air_quality function
...
# Apply the categorize_air_quality function to predictions
predictions['Air Quality'] = predictions.apply(categorize_air_quality, axis=1)

# Reorder columns in predictions DataFrame
...
# Define the give_advice function
def give_advice(row):
    pm25 = row['PM2.5 Density (ug/m3)']
    co = row['CO (ppm)']
    co2 = row['CO2 (ppm)']
...
# Apply the give_advice function to predictions
predictions['Advice'] = predictions.apply(give_advice, axis=1)

# Define the function to print predictions
def print_predictions(predictions):
    ...
    for index, row in predictions.iterrows():
        print(f"Prediksi Kualitas Udara {time_mapping[row['Time of Day']]}: {row['Air Quality']}")
        print(f"PM2.5: {row['PM2.5 Density (ug/m3)']:.2f}, CO: {row['CO (ppm)']:.2f}, CO2: {row['CO2 (ppm)']:.2f}")
        print("Saran:")
        for advice in row['Advice']:
            print(f"- {advice}")
        print()

# Define the function to load models and predict
def load_models_and_predict():
    new_data['Date and Time'] = pd.to_datetime(new_data['Date and Time'])
    new_data['Time of Day'] = new_data['Date and Time'].dt.hour.apply(categorize_time_of_day)
    new_data['Date'] = new_data['Date and Time'].dt.date
    grouped_new = new_data.groupby(['Date', 'Time of Day']).mean().reset_index()

    predictions = pd.DataFrame()
    for feature in features:
        min_value = min_values[feature]
        model_fit = models[feature]
        forecast = model_fit.get_forecast(steps=4)
        forecast_mean = forecast.predicted_mean
        forecast_mean = np.maximum(forecast_mean, min_value)
        predictions[feature] = forecast_mean

    future_dates = pd.date_range(start=grouped_new['Date'].max() + pd.Timedelta(days=1), periods=4, freq='D')
    future_time_of_day = ['Pagi', 'Siang', 'Sore', 'Malam']
    predictions['Date'] = future_dates.repeat(4)[:len(predictions)]
    predictions['Time of Day'] = future_time_of_day * (len(predictions) // 4)
